...
# NOTE: What type of error does python throw if you try to turn a non-number string into an integer? Peter's answer: ValueError
# Test it out (or google it!) to see which one. Specifically catch that exception in your code.

def validate_user_input():
    while True:
        try:
            ent_num = int(input('Please enter a number '))
            if type(ent_num)!=int:
                continue
            else:
                return ent_num
